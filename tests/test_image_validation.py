...
from .conftest import LARGE_TEST_DATA_SIZE, SAMPLE_IMAGE_BYTES
...
PROVIDER_INTEGRATION_MATRIX = [
    ("providers.gemini", "GeminiModelProvider", "providers.gemini.logger"),
...
class TestImageValidation:
    ...
    def test_validate_data_url_errors(self, image_provider, validation_test_data, url_key, error_key):
        ...
        ImageValidationAssertions.assert_data_url_error(
            image_provider, validation_test_data["urls"][url_key], validation_test_data["errors"][error_key]
        )
    ...
    def test_validate_image_size_limits(
        self, image_provider, temp_file_factory, max_size_mb, file_size_mb, should_succeed
    ):
        ...
        if should_succeed:
            ...
            assert f"Image too large: {file_size_mb}.0MB (max: {max_size_mb}MB)" in str(excinfo.value)
    ...
    def test_validate_all_supported_formats(self, image_provider, temp_file_factory, ext, expected_mime):
        ...
        ImageValidationAssertions.assert_image_validation_success(image_provider, image_file, data, expected_mime)
...
class TestProviderIntegration:
    ...
    def test_vertex_ai_provider_uses_validation(self, mock_init, mock_auth):
        ...
        # Import here since we're actually testing the provider
        try:
            ...
            pytest.skip("Vertex AI dependencies not available")
