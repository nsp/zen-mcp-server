...
class SimulatorTestFactory:
    ...
    def create_conversation_test(
        simulator: BaseSimulatorTest, tool_name: str = "chat", model: str = "flash", files: Optional[list[str]] = None
    ) -> dict[str, Any]:
    ...
    def create_file_setup(
        simulator: BaseSimulatorTest, additional_files: Optional[dict[str, str]] = None
    ) -> dict[str, str]:
    ...
    def validate_continuation_flow(
        response1: Optional[str], continuation_id: Optional[str], response2: Optional[str]
    ) -> bool:
        ...
        return (
            ...
            and len(response1.strip()) > 0
            and isinstance(continuation_id, str)
            and len(continuation_id.strip()) > 0
            and isinstance(response2, str)
            and len(response2.strip()) > 0
        )
...
class TestSimulatorFactory:
    ...
    def test_conversation_flow_factory(
        self, simulator_factory, conversation_test_data, tool_name, initial_prompt, continuation_prompt, model
    ):
    ...
    def test_file_deduplication_factory(
        self, simulator_factory, tool_name, files_initial, files_continuation, should_deduplicate
    ):
    ...
    def test_continuation_validation_factory(
        ...
        is_valid = SimulatorTestFactory.validate_continuation_flow(response1, continuation_id, response2)
