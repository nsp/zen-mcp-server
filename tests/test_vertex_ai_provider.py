...
PROVIDER_INIT_SCENARIOS = [
    ...
    ({"VERTEX_AI_PROJECT_ID": "test-project", "VERTEX_AI_LOCATION": "europe-west4"}, "test-project", "europe-west4", True),
]
...
class TestUnifiedVertexAIProvider:
    ...
    def test_provider_initialization(self, env_vars, expected_project, expected_location, should_succeed):
        ...
        with patch("google.auth.default") as mock_auth, \
            ...
            mock_auth.return_value = (Mock(), expected_project)
    ...
    def test_model_capabilities(self, mock_vertex_provider, model_name, expected_context, expected_images, is_claude):
        ...
        resolved_name = provider._resolve_model_name(model_name)
    ...
    @pytest.mark.parametrize("error_class,is_retryable", [
        ("Unauthenticated", False),  # Auth errors not retryable
        ("ServiceUnavailable", True),  # Service errors retryable
        ("ResourceExhausted", True),  # Rate limits retryable
        ("DeadlineExceeded", True),  # Timeout retryable
    ])
    def test_error_handling(self, mock_vertex_provider, error_class, is_retryable):
        """Test error handling for various scenarios."""
        from google.api_core import exceptions as google_exceptions
        
        provider = mock_vertex_provider()
        error = getattr(google_exceptions, error_class)("Test error")
        assert provider._is_error_retryable(error) == is_retryable
